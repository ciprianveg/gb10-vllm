#!/usr/bin/env python3
"""Assemble a zero-copy GLM-5.3 Int4-Int8Mix (Tech2wild) + Baseten GLM-5V vision checkpoint.

Identical logic to glm-5.2/v18-vision/scripts/assemble_quanttrio_glm5v.py — GLM-5.3
shares GLM-5.2's architecture (glm_moe_dsa, hidden 6144, same vocab), so the same
composite method transfers unchanged. The text backbone (compressed-tensors) + vision
tower are symlinked (zero-copy); only generated files (config.json,
model.safetensors.index.json, preprocessor_config.json, chat_template.jinja,
GLM5V_COMPOSITE.json) are written. No model weights are altered.
"""
