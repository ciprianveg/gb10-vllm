...
class Worker(WorkerBase):
    ...
    def compile_or_warm_up_model(self) -> CompilationTimes:
        ...
        if self.use_v2_model_runner and os.environ.get("VLLM_K3_SKIP_WARMUP", "0") != "1":
            # V2: Run full execute_model + sample_tokens to JIT compile triton kernels.
            # V4PLUS-V6C: VLLM_K3_SKIP_WARMUP=1 skips the post-capture warmup —
            # the KDA kernels (causal_conv1d / fused_recurrent_kda) fault in this
            # eager step after graph capture (fork #396 class, pinned via
            # CUDA_LAUNCH_BLOCKING). Kernels JIT on first real requests instead.
            if os.environ.get("VLLM_K3_SKIP_WARMUP", "0") == "1":
                logger.info("VLLM_K3_SKIP_WARMUP=1: skipping post-capture warmup_kernels "
                            "(KDA kernels JIT on first requests).")
            else:
